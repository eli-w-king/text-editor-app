/**
 * EditorPage -- Main document editing page.
 *
 ...
 *
 * LLM transition affordances:
 *   - SkeletonLoader shows glass shimmer lines while waiting for AI response
 *   - StreamingTextTransition renders streamed text with word-by-word fade-in
 *   - LLMThinkingIndicator shows 3 pulsing dots inline after the cursor
 *   - ImageGenerationShimmer shows frosted glass pulse for image generation
 *   - TransitionOverlay provides ultra-subtle ambient dim during AI ops
 *
...
import {
  ...
  StreamingTextTransition,
  SkeletonLoader,
  ImageGenerationShimmer,
  TransitionOverlay,
} from '../components/LLMTransitions';
...
function getPalette(dark: boolean): ThemePalette {
  if (dark) {
    return {
      ...
      spinnerTop: 'rgba(255, 255, 255, 0.6)',
    };
  }
  return {
    ...
    dangerText: '#FF3B30',
    spinnerBorder: 'rgba(0, 0, 0, 0.08)',
    spinnerTop: 'rgba(0, 0, 0, 0.4)',
  };
}
...
//  AI State Controller
//
//  Exposes setters for the LLM transition state. When the AI integration
//  layer is wired up, it will call these setters to drive the transition
//  affordances (skeleton loaders, streaming text, thinking indicator, etc.).
//
//  Currently the AI state defaults to inactive (all false / empty string).
//  The setters are captured in a ref object to avoid triggering re-renders
//  while still satisfying TypeScript's noUnusedLocals rule.
// ---------------------------------------------------------------------------
...
export default function EditorPage() {
  ...
  // ---- LLM transition state ----
  // These state values drive the visibility of transition affordances.
  // The setters are stored in a ref so the AI integration layer can call
  // them when it is wired up. This pattern satisfies noUnusedLocals while
  // keeping the state reactive.
  const [aiGenerating, setAiGenerating] = useState(false);
  const [streamingText, setStreamingText] = useState('');
  const [imageGenerating, setImageGenerating] = useState(false);

  // Store setters in a ref -- the AI integration layer will use this to
  // drive transition states. Exposed for future use.
  const aiSettersRef = useRef<AiStateSetters>({
    setAiGenerating,
    setStreamingText,
    setImageGenerating,
  });
  aiSettersRef.current = {
    setAiGenerating,
    setStreamingText,
    setImageGenerating,
  };
  ...
  // ---- LLM transition area styles ----
  const aiAreaStyle: React.CSSProperties = {
    marginTop: 12,
    padding: '4px 0',
  };
  ...
  const spinnerStyle: React.CSSProperties = {
    width: 24,
    height: 24,
    border: `2px solid ${p.spinnerBorder}`,
    borderTopColor: p.spinnerTop,
    borderRadius: '50%',
  ...
  const spinKeyframes = `@keyframes editor-spin { to { transform: rotate(360deg); } }`;
  ...
  return (
    <div style={pageStyle}>
      ...
      {/* Editor area with transition overlay */}
      <div style={editorAreaStyle}>
        {/* TransitionOverlay -- ultra-subtle ambient dim during AI ops */}
        <TransitionOverlay isDark={isDark} active={aiGenerating || imageGenerating} />
        ...
        {/* LLM Thinking Indicator -- shows inline after the editor content */}
        {aiGenerating && (
          <div style={aiAreaStyle}>
            <LLMThinkingIndicator isDark={isDark} />
          </div>
        )}

        {/* SkeletonLoader -- shows when AI is generating but no text has streamed yet */}
        {aiGenerating && !streamingText && (
          <div style={aiAreaStyle}>
            <SkeletonLoader isDark={isDark} lines={3} />
          </div>
        )}

        {/* StreamingTextTransition -- shows when AI text is actively streaming in */}
        {aiGenerating && streamingText && (
          <div style={aiAreaStyle}>
            <StreamingTextTransition text={streamingText} isDark={isDark} />
          </div>
        )}

        {/* ImageGenerationShimmer -- shows when an AI image is being generated */}
        {imageGenerating && (
          <div style={{ ...aiAreaStyle, marginTop: 20 }}>
            <ImageGenerationShimmer isDark={isDark} />
          </div>
        )}
      </div>
