...
const sanitizeModelContent = (rawInput) => {
  ...
  cleaned = cleaned
    .replace(
      /^\s*(?:according to|based on|from|per|via|as per|the answer is|it is|this is|that is|it's|that's|here is|here's|the)[,:\s]*/gi,
      "",
    )
    .replace(
      /^\s*(?:search results?|sources?|web search|results? show|i found|looking up)[,:\s]*/gi,
      "",
    )
    .replace(
      /^\s*(?:the (?:acts?|artists?|bands?|members?|names?|answer) (?:under|include|is|are|would be))[,:\s]*/gi,
      "",
    )
  ...
  cleaned = cleaned
    .replace(
      /\s*\[[^\]]*\b\w+\.\s*(?:com?|net|org|io|ai|app|news|tv|fm|uk|us|au|de|fr|jp|co)\b[^\]]*\]/gi,
      "",
    )
...
const textToBlocks = (text) => {
  ...
  const combinedRegex = new RegExp(
    `(${IMAGE_MARKER_REGEX.source})|(${IMAGE_LOADING_PLACEHOLDER_REGEX.source})`,
    "g",
  );
  let lastIndex = 0;
  let match;
...
function EditorScreen() {
  ...
  const getRandomColorFamily = () => {
    return colorFamilyNames[
      Math.floor(Math.random() * colorFamilyNames.length)
    ];
  };
  ...
  const addColorDot = (latencyMs = 500, tokens = 20) => {
    ...
    const clampedLatency = Math.max(
      minLatency,
      Math.min(latencyMs, maxLatency),
    );
    const normalized =
      (clampedLatency - minLatency) / (maxLatency - minLatency);
    ...
    const targetOpacity = 0.05 + Math.random() * 0.1; // 0.05-0.15 opacity
    ...
    try {
      ...
      const newNote = {
        ...
        colorDots: colorDots.map((dot) => ({
          ...
          opacityValue:
            typeof dot.opacity === "object"
              ? dot.opacity._value || 0.8
              : dot.opacity || 0.8,
        })),
      };
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (
        llmStatus === "connected" &&
        textOnly.length > 20 &&
        Math.abs(textOnly.length - lastTitleGenLength) > 50
      ) {
        generateTitle();
      }
    }, 3000); // Debounce 3s
  ...
  const generateTitle = async () => {
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a helpful assistant. Summarize the user's text into a short, concise title (3-5 words max). Do not use quotes. IMPORTANT: The title MUST be in the same language as the user's text. If they write in Spanish, title in Spanish. If French, title in French. Match their language exactly.",
            },
      ...
      if (data.choices && data.choices.length > 0) {
        const newTitle = data.choices[0].message.content
          .trim()
          .replace(/^["']|["']$/g, "");
  ...
  const handleTextChange = (newText) => {
    ...
    if (doubleMatch) {
      ...
      if (prefix.trim() === "") {
        if (llmStatus !== "connected") {
          Alert.alert(
            ...
            "Tap the Cloud button in the menu to add your OpenRouter API key.",
          );
      ...
      const allFillPositions = [...slashPositions, doubleTriggerPos].sort(
        (a, b) => a - b,
      );
  ...
  const triggerImageCommand = async (query, placeholderId) => {
    ...
    try {
      ...
      const noteContext = stripImageMarkers(
        text.replace(placeholderId, "").trim(),
      ).slice(0, 500);
      let resolvedQuery = query;

      if (
        noteContext.length > 0 &&
        /\b(he|she|her|him|his|they|them|it|this|that)\b/i.test(query)
      ) {
        try {
          const data = await makeApiCall(
            "/chat/completions",
              ...
              messages: [
                {
                  role: "system",
                  content:
                    'The user is writing a note and wants to insert an image. Resolve any pronouns or vague references in their image request using the note context. Return ONLY the resolved description (e.g. "Sam Altman smiling"). No explanation.',
                },
                {
                  role: "user",
                  content: `Note context: "${noteContext}"\n\nImage request: "${query}"`,
                },
              ],
              temperature: 0.1,
              max_tokens: 50,
          ...
          const resolved = coerceMessageContent(
      ...
      if (!imageUri) {
        console.log(
          "Image search returned no results, falling back to Gemini image generation...",
        );
      ...
      if (imageUri) {
        ...
        Alert.alert(
          "Image Not Found",
          `Could not find or generate an image for "${query}".`,
        );
      }
    } catch (error) {
  ...
  const findAllSlashPlaceholders = (text) => {
    ...
    const isInsideImgMarker = (idx) =>
      imgRanges.some(([start, end]) => idx >= start && idx < end);
  ...
  const triggerBatchFill = async (fullText, fillPositions) => {
    ...
    try {
      ...
      if (data.choices && data.choices.length > 0) {
        const rawContent = coerceMessageContent(
          data.choices[0].message.content,
        );
                  ...
                  return (
                    <TextInput
                      ...
                      onChangeText={(newText) =>
                        handleBlockTextChange(index, newText)
                      }
                      placeholder={
                        index === 0 && contentBlocks.length === 1
                          ? "Type / for blanks, // to fill them, /image query/"
                          : isAfterImage
                            ? "Keep typing..."
                            : undefined
                      }
        ...
        >
          ...
          <SafeAreaView style={{ flex: 1 }}>
            ...
            <ScrollView
              ...
              contentContainerStyle={[
                savedNotesOverlayStyles.notesListContent,
                { paddingTop: 146 },
              ]}
            >
              {savedNotes.length === 0 ? (
                ...
                savedNotes.slice(0, 50).map((note) => {
                  return (
                    ...
                    >
                      ...
                      >
                        {note.content
                          ? stripImageMarkers(note.content)
                              .replace(/\[IMG\|\|.*?\|\|.*?\]/g, "")
                              .trim() || "(image)"
                          : "No content"}
                      </Text>
